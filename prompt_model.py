...
class PromptTuningModel(nn.Module):
    ...
    def __init__(self, 
        ...
        if initialize_from_vocab:
            # Initialize from vocabulary tokens
            vocab_size = self.config.vocab_size
            indices = torch.randint(0, vocab_size, (num_virtual_tokens,))
            sampled_embeddings = self.gpt2.transformer.wte.weight[indices].clone().detach()
            self.prompt_embeddings = nn.Parameter(sampled_embeddings)
        else:
            # Random initialization
            self.prompt_embeddings = nn.Parameter(
                torch.randn(num_virtual_tokens, self.embedding_size)
            )
